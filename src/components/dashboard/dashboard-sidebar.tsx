...
import { dashboardNavItems, type NavItem } from "@/constants/dashboardNavItems";
...
export function DashboardSidebar({
  ...
  // Auto-expand any section whose child (or nested route) is active
  const [openSections, setOpenSections] = useState<Set<string>>(() => {
    const set = new Set<string>();
    dashboardNavItems.forEach((item) => {
      if (isItemActive(item)) set.add(item.path);
    });
    return set;
  });
