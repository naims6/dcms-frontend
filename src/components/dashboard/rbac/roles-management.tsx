    ...
    <PermissionGuard requiredPermission="roles:read">
      <div className="space-y-6 animate-in fade-in-0 duration-300">
        ...
        <DashboardPageHeader
          ...
          description="Create, configure, and assign access level privileges across system roles (TanStack Query Managed)"
          icon={ShieldCheck}
                    ...
                    <div className="flex flex-wrap gap-1 max-h-24 overflow-y-auto p-1 bg-muted/20 rounded-md border border-border/40">
                      {(role.permissions ?? []).length === 0 ? (
                        ...
                        (role.permissions ?? []).map((p) => (
                          <Badge key={p.id} variant="secondary" className="text-[10px] py-0 px-1.5">
                            {p.name}
                          </Badge>
                        ))
                      )}
        ...
        <Dialog open={isCreateOpen} onOpenChange={setIsCreateOpen}>
          <DialogContent className="sm:max-w-lg max-h-[90vh] flex flex-col">
            ...
            <div className="space-y-4 py-2 flex-1 overflow-y-auto">
              <div className="space-y-1.5">
                <label className="text-xs font-semibold">Role Name (Unique Identifier)</label>
                <Input
              ...
              <div className="space-y-2">
                <label className="text-xs font-semibold flex items-center justify-between">
                  ...
                  <span className="text-[10px] text-muted-foreground">Check all that apply</span>
                </label>
                  ...
                  {permissions.map((p) => (
                    ...
                    >
                      <Checkbox
                        ...
                        onCheckedChange={() => togglePermissionSelection(p.name)}
                      />
