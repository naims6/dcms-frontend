        ...
        <SheetContent side="right" className="w-[85vw] sm:w-[400px] p-0 flex flex-col border-l-0 shadow-2xl">
          ...
          <div className="flex flex-col flex-1 overflow-y-auto p-6">
            <nav className="flex flex-col gap-2.5">
              {navMenuItems.map((link) => {
                ...
                return (
                  ...
                  >
                    <span className={cn("transition-transform duration-300", isActive ? "scale-105 font-semibold" : "")}>
                      {t(link.labelKey)}
                    </span>
                  </Link>
                );
            ...
            <div className="mt-8 flex flex-col gap-3">
              <Button variant="outline" className="w-full justify-center h-12 rounded-xl text-base font-medium" asChild>
                <Link href="/login" onClick={() => setOpen(false)}>{t('login')}</Link>
              </Button>
              <Button className="w-full justify-center h-12 rounded-xl text-base font-medium bg-primary hover:bg-primary/90 text-primary-foreground shadow-lg shadow-primary/20" asChild>
          ...
          <div className="p-6 bg-neutral-50/50 dark:bg-neutral-900/50 mt-auto border-t border-neutral-100 dark:border-neutral-800/50 flex flex-col gap-4 relative">
            <div className="flex items-center justify-between">
               <span className="text-sm font-medium text-neutral-600 dark:text-neutral-400">{t('toggleTheme') || "Theme"}</span>
               <ThemeToggle />
            </div>
          </div>
        </SheetContent>
